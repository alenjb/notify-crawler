...
# 주키퍼 설치
wget -q https://downloads.apache.org/zookeeper/zookeeper-3.9.2/apache-zookeeper-3.9.2-bin.tar.gz || { echo "주키퍼 다운로드에 실패했습니다."; exit 1; }
tar -xzf apache-zookeeper-3.9.2-bin.tar.gz || { echo "주키퍼 압축 해제에 실패했습니다."; exit 1; }
mv apache-zookeeper-3.9.2-bin /opt/zookeeper || { echo "주키퍼 이동에 실패했습니다."; exit 1; }
rm apache-zookeeper-3.9.2-bin.tar.gz || { echo "주키퍼 압축 파일 삭제에 실패했습니다."; exit 1; }
