...
public class CrawlerController {
    ...
    public void getComNotice() throws InterruptedException, ParseException {
        ...
        if(newNoticeCount > 0){
            ...
            kafkaProducer.sendMessage("공통 공지사항 저장 완료");
        }
    }
    ...
    public void getAllMajorNotice() throws InterruptedException, ParseException {
        ...
        for (NoticeType noticeType : noticeTypes) {
            // 공통 공지사항이면 지나가기
            if(noticeType == NoticeType.COM) continue;
            // 새 글의 개수
            int newNoticeCount = crawlerService.getNewNoticeCount(
                    noticeType,
                    driver,
            ...
            if(newNoticeCount > 0){
                ...
                log.info("==== "+ noticeType + "학과 새 공지사항 저장 완료 시각: "+ String.valueOf(LocalDateTime.now())+"====");
                kafkaProducer.sendMessage("학과 공지사항 저장 완료");
