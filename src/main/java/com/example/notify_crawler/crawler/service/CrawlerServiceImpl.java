...
public class CrawlerServiceImpl implements CrawlerService{
    ...
    public int getNewNoticeCount(NoticeType noticeType, WebDriver driver, String [][] top2) throws InterruptedException, ParseException {
        ...
        titlesAndDates = getTitlesAndDatesOfNoticeFromPageNum(driver, 1, noticeType);
    ...
    public int findNewNoticeOrder(String[][] top2, WebDriver driver, NoticeType noticeType) throws InterruptedException, ParseException {
        ...
        for(int i=1; i<=10; i++){
            ...
            if(noticeType == NoticeType.COM){
                pageButton = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".paging > li:nth-of-type(" + i + ")")));
            }else {
            ...
            titlesAndDates = getTitlesAndDatesOfNoticeFromPageNum(driver, i, noticeType);
            ...
            for (int k = 0; k < titlesAndDates.titles().size(); k++) {
                if (titlesAndDates.titles().get(k).equals(firstTitle) && StringDates.get(k).equals(firstDate)) {
            ...
            for (int k = 0; k < titlesAndDates.titles().size(); k++) {
                if (titlesAndDates.titles().get(k).equals(secondTitle) && StringDates.get(k).equals(secondDate)) {
                    return (k - 1) * noticeType.getNoticeSizePerPage() + k;
                }
            }
        }
     ...
     */
    @Override
    public TitlesAndDates getTitlesAndDatesOfNoticeFromPageNum(WebDriver driver, int pageNum, NoticeType noticeType) throws ParseException {
        ...
        WebElement pageButton;
        if(noticeType == NoticeType.COM){
            pageButton = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".paging > li:nth-of-type(" + pageNum + ")")));
        }else {
            pageButton = wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector(".paging-wrap > li:nth-of-type(" + pageNum + ")")));
        }
        pageButton.click();
